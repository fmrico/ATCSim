...
Airport::generate_storm() {
  ...
  // Storm Pos
  angle = toRadians(static_cast<float>(rand() % 360 - 180));
  x = fabs(AIRPORT_DISTANCE_MAX*1.5 * cos(angle));  // Only positive, for GyV3D!!!!!!!!!!!
  y = AIRPORT_DISTANCE_MAX*1.5 * sin(angle);
  z = (static_cast<float>(rand()) / RAND_MAX)*(STORM_MIN_ELEVATION-STORM_MIN_ELEVATION)+STORM_MIN_ELEVATION;

  // Storm height
  height = (static_cast<float>(rand()) / RAND_MAX)*(STORM_MAX_HEIGHT-STORM_MIN_HEIGHT)+STORM_MIN_HEIGHT;

  // Radius
  rad =  (static_cast<float>(rand()) / RAND_MAX)*(STORM_MAX_RAD-STORM_MIN_RAD)+STORM_MIN_RAD;

  // Bearing
  bearing = toRadians(static_cast<float>(rand() % 360 - 180));

  // Speed
  speed = (static_cast<float>(rand()) / RAND_MAX)*(STORM_MAX_SPEED-STORM_MIN_SPEED)+STORM_MIN_SPEED;
  Position ipos(x, y, z);
...
Airport::generate_flight() {
  ...
  z = FLIGHT_HEIGHT + static_cast<float>(rand() % 2000);
...
Airport::step() {
  ...
  delta_t =(static_cast<float>(ta-tb)) /1000000.0;
  last_ts = tv;
...
((*it)->getSpeed() < LANDING_SPEED)) {
  ...
  points += static_cast<int>(*it)->getPoints();
